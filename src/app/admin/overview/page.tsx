...
import { Users, LayoutGrid, CheckCircle2, ChevronRight, GraduationCap, UserCircle, Send, Megaphone } from "lucide-react";
  ...
  const handleSendNotification = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(`Notification sent to ${result.count || 'all'} students!`);
        setNotifMessage("");
      } else {
  ...
  return (
    <div className="h-full w-full">
      <div className="glass-card rounded-2xl p-6 space-y-6">
        ...
        {/* Send Custom Notification — Full Width */}
        <div className="space-y-3 pt-2">
          ...
          <div className="space-y-3">
            <textarea
              ...
              className="w-full h-24 rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 resize-none"
            />
            <button
              onClick={handleSendNotification}
              disabled={sendingNotif || !notifMessage.trim()}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white transition-all hover:bg-indigo-700 active:scale-95 disabled:opacity-50 shadow-lg shadow-indigo-200"
            >
              <Send size={14} />
