...
import { Layers, Calendar as CalendarIcon, ChevronLeft, ChevronRight, CheckCircle, XCircle } from "lucide-react";
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 h-96 rounded-2xl bg-white/40 animate-pulse" />
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      {classes.length === 0 ? (
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          {/* LEFT — Classes List (vertical series) */}
            ...
            {classes.map((cls, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-full glass-card rounded-2xl p-4 text-left hover:shadow-lg transition-all duration-300 group cursor-pointer border-2 ${
                    isSelected
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`h-10 w-10 rounded-xl bg-gradient-to-br ${classColors[i % classColors.length]} flex items-center justify-center text-white shadow-lg shrink-0 group-hover:scale-110 transition-transform`}>
                      <Layers size={18} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 className="font-bold text-slate-800 text-sm truncate">{cls.name}</h3>
                      <p className="text-[10px] text-slate-400 font-medium">{cls.department || "Department"} · {cls.year}</p>
                    </div>
                  </div>
          ...
          <div className="flex-1 min-w-0">
            {selectedClass ? (
              <div className="glass-card rounded-2xl p-5 animate-in fade-in duration-300">
                {/* Calendar Header */}
                ...
                <div className="grid grid-cols-7 gap-1">
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    if (isPresent && isAbsent) {
                      ...
                      statusIcon = <CheckCircle size={12} className="text-emerald-500 mt-1" />;
                    } else if (isAbsent) {
                      bgClass = "bg-rose-50 border-rose-200";
                      textClass = "text-rose-700 font-bold";
                      statusIcon = <XCircle size={12} className="text-rose-500 mt-1" />;
                    }
                    ...
                    return (
                      <div key={day} className={`relative flex flex-col items-center p-1.5 sm:p-2 rounded-xl border transition-all min-h-[56px] sm:min-h-[64px] ${bgClass}`}>
                ...
                {/* Legend */}
                <div className="mt-4 flex flex-wrap gap-4 items-center justify-center text-[10px] font-semibold text-slate-500 uppercase tracking-wider bg-white py-2.5 rounded-xl border border-slate-100">
              ...
              <div className="glass-card rounded-2xl p-12 text-center h-full flex flex-col items-center justify-center">
                ...
                <p className="text-slate-400 text-sm">Select a class to view calendar</p>
              </div>
            )}
          </div>
        </div>
